    ...
    async def run(self, target_url: str, endpoints: List[Dict], config: Dict, baseline_cache=None) -> List[Dict]:
        ...
        async with httpx.AsyncClient(verify=False, timeout=8.0) as client:
            for ep in auth_endpoints:
                ...
                # Baseline: does this endpoint require auth at all? If a request
                # with NO Authorization header already succeeds, the endpoint is
                # public by design — forging a JWT for it isn't a real bypass, so
                # skip it entirely rather than reporting a false "JWT bypass".
                try:
                ...
                if no_auth_resp.status_code in (200, 201):
                    continue
                auth_normally_enforced = no_auth_resp.status_code in (401, 403)
                ...
                try:
                    ...
                    if resp.status_code in (200, 201):
                        signals = ["forged_token_accepted"]
                        if auth_normally_enforced:
                            signals.append("auth_normally_enforced")
                ...
                for secret, weak_jwt in weak_jwts.items():
                    try:
                        ...
                        if resp.status_code in (200, 201):
                            signals = ["forged_token_accepted", "known_weak_secret"]
                            if auth_normally_enforced:
                                signals.append("auth_normally_enforced")
                            findings.append(self.build_finding(
