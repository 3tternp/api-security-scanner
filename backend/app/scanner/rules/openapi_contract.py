...
from app.scanner.allowlists import is_pii_field
    ...
    async def run(self, target_url: str, endpoints: List[Dict], config: Dict, baseline_cache=None) -> List[Dict]:
        ...
        for endpoint in endpoints:
            ...
            if method in ['POST', 'PUT', 'DELETE']:
                # Postman collections declare auth per-request via an 'auth'
                # block (e.g. {"type": "basic", ...}) rather than OpenAPI's
                # 'security' array — check the field that actually applies
                # to this endpoint's source format instead of assuming
            ...
            # Check request parameters and response schemas for PII field names
            # (whole-token matching via app.scanner.allowlists.is_pii_field,
            # not a bare substring scan — avoids false positives like
            # "automobile_type" matching "mobile" or "adobe_key" matching "dob").

            # Check Request Body properties
            if 'content' in request_body:
                for content_type, media in request_body['content'].items():
                    schema = media.get('schema', {})
                    self._check_schema_for_pii(schema, path, method, findings, "Request Body")
            ...
            for status_code, response_def in responses.items():
                ...
                for content_type, media in response_content.items():
                    schema = media.get('schema', {})
                    self._check_schema_for_pii(schema, path, method, findings, f"Response {status_code}")

        return findings

    def _check_schema_for_pii(self, schema, path, method, findings, location):
        properties = schema.get('properties', {})
        for prop_name, prop_def in properties.items():
            if is_pii_field(prop_name):
                ...
                findings.append(self.build_finding(
                    ...
                    signals=["pii_field_name_allowlist_match"],
                    confidentiality="High",
                    integrity="None",
                    availability="None"
                ))
