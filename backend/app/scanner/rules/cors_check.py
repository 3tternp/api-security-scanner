                ...
                for origin in self.TEST_ORIGINS:
                    for method in ["OPTIONS", "GET"]:
                        try:
                            ...
                            # Arbitrary origin reflection — only a *critical* issue when
                            # paired with confirmed Allow-Credentials: true (that combo
                            # lets any origin make authenticated requests). Reflection
                            # alone, without confirmed credentials, is downgraded.
                            if acao == origin and origin not in ("null",):
                                ...
                                credentials_confirmed = acac == "true"
                                ...
                                findings.append(self.build_finding(
                                    ...
                                    endpoint=path,
                                    method=method,
                                    severity="high" if credentials_confirmed else "medium",
                                    confidence="high" if credentials_confirmed else "low",
                                    proof_of_concept=(
                                        f"Request with Origin: {origin} returned "
                                        f"Access-Control-Allow-Origin: {acao}"
                                        + (" with credentials allowed." if credentials_confirmed else ".")
                                    ),
                                ))
