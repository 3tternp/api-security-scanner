...
def generate_docx_report(scan: ScanJob, results: list, output_path: str):
    ...
    if severity_counts["Critical"] > 0:
        ...
        run = callout.add_run(
            f"⚠ {severity_counts['Critical']} CRITICAL finding"
            f"{'s' if severity_counts['Critical'] != 1 else ''} require immediate remediation "
            f"before this system is considered safe for production use."
        )
        run.bold = True
        run.font.size = Pt(11)
        run.font.color.rgb = _severity_color("Critical")
    ...
    if not results:
        ...
        # Most urgent first — a critical finding shouldn't be buried on
        # page 12 just because a low-severity header check happened to run
        # earlier in the scan.
        SEVERITY_RANK = {"critical": 0, "high": 1, "medium": 2, "low": 3, "info": 4}
        unique_results.sort(key=lambda r: SEVERITY_RANK.get((r.severity or "info").lower(), 5))

        def _owasp_category(finding) -> str:
            d = finding.details
            if isinstance(d, dict) and d.get("owasp"):
                return d["owasp"]
            return "Uncategorized"

        OWASP_ORDER = [f"API{i}" for i in range(1, 11)]

        def _category_sort_key(cat: str) -> int:
            for i, prefix in enumerate(OWASP_ORDER):
                if cat.startswith(prefix):
                    return i
            return len(OWASP_ORDER)

        grouped: Dict[str, list] = {}
        for r in unique_results:
            grouped.setdefault(_owasp_category(r), []).append(r)
        ...
        for category in sorted(grouped.keys(), key=_category_sort_key):
            ...
            for finding in grouped[category]:
                idx = _render_finding(doc, finding, idx)
...
def _render_finding(doc, finding, idx: int) -> int:
    """Render one finding's heading + detail table into doc. Returns idx + 1."""
    from docx.shared import Pt
...
def get_dashboard_stats(
    ...
    try:
        ...
        RULE_TO_OWASP: Dict[str, str] = {
            ...
            "FINGERPRINT":      "API8",
            "CRYPTO-ORACLE":    "API8",
            "TRACE-METHOD-001": "API8",
            "ERROR-DISCLOSURE": "API8",
            "OPENAPI-CONTRACT": "API9",
            "GRAPHQL-INTROSPECTION": "API9",
