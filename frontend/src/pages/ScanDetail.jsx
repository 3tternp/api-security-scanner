...
const CONFIDENCE = {
  ...
  medium: { label: 'Medium Confidence', badge: 'bg-slate-200 text-slate-600' },
  low: { label: 'Low Confidence', badge: 'bg-gray-100 text-gray-500' },
}
...
const getConfidenceConfig = (confidence) =>
  CONFIDENCE[(confidence || '').toLowerCase()] || CONFIDENCE.medium
...
const FindingCard = ({ result, onStatusChange }) => {
  ...
  const confCfg = getConfidenceConfig(result.confidence)
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span
              className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded-full ${confCfg.badge}`}
              title="How many corroborating signals confirmed this finding"
            >
              {confCfg.label}
            </span>
            <select
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-4 py-4 space-y-4 bg-gray-50">
          ...
          {result.signals && result.signals.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Corroborating Signals</p>
              <div className="flex flex-wrap gap-1.5">
                {result.signals.map((s) => (
                  <span key={s} className="text-[11px] font-mono bg-white border border-gray-200 rounded px-1.5 py-0.5 text-gray-700">
                    {s}
                  </span>
                ))}
              </div>
...
const ScanDetail = () => {
  ...
  const filteredResults = (results || []).filter((r) => {
    ...
    const confMatch = confidenceFilter === 'all' || conf === confidenceFilter
    return sevMatch && stMatch && confMatch
  })
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1 flex-wrap">
            ...
            {['all', 'high', 'medium', 'low'].map((c) => (
              ...
              >
                {c === 'all' ? 'All' : CONFIDENCE[c].label.replace(' Confidence', '')}
              </button>
            ))}
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-10 text-center">
            {results?.length === 0 ? (
              ...
              <>
                ...
                <button
                  onClick={() => { setSeverityFilter('all'); setStatusFilter('all'); setConfidenceFilter('all') }}
